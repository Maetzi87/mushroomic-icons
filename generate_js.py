...
SVG = "svg"
OUTFILE = join("dist", "mushic-icons.js")
...
template = Template(
    """const MUSHIC_ICONS_MAP = $icons;

async function getIcon(name) {
  return {path: MUSHIC_ICONS_MAP[name]?.path};
}
async function getIconList() {
  return Object.entries(MUSHIC_ICONS_MAP).map(([icon, content]) => ({
    name: icon,
...
window.customIcons["mushic"] = { getIcon, getIconList };

window.customIconsets = window.customIconsets || {};
window.customIconsets["mushic"] = getIcon;
"""
)
